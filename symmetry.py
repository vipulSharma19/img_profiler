...
# bf = cv2.BFMatcher()
#
# def bilateral_syemmetry(img, title: str, show_detail = False):
#     """
#     Main function
#
...
#         gray_image = cv2.cvtColor(image_path, cv2.COLOR_RGB2GRAY)
#
#         self.reflected_image = np.fliplr(gray_image)  # Flipped version of the image
#         self.kp1, self.des1 = sift.detectAndCompute(image_path, None)
#         self.kp2, self.des2 = sift.detectAndCompute(self.reflected_image, None)
#
#
...
class Mirror_Symmetry_detection:
    ...
    def _read_color_image(self, image_path):
        """
        Convert the image into the array/matrix with original color
        """
        image = cv2.imread(image_path)  # convert the image into the array/matrix
        b, g, r = cv2.split(image)  # get b,g,r
        image = cv2.merge([r, g, b])  # switch it to rgb
